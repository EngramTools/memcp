...
async fn test_recall_basic(pool: PgPool) {
    ...
    let result = engine.recall(&query_emb, Some("session-basic".to_string()), false).await.unwrap();
...
async fn test_recall_session_dedup(pool: PgPool) {
    ...
    let result1 = engine
        .recall(&query_emb, Some("session-dedup".to_string()), false)
        .await
        .unwrap();
    ...
    let result3 = engine
        .recall(&query_emb, Some("session-other".to_string()), false)
        .await
        .unwrap();
...
async fn test_recall_reset_clears_session(pool: PgPool) {
    ...
    let result1 = engine
        .recall(&query_emb, Some("session-reset".to_string()), false)
        .await
        .unwrap();
    ...
    let result3 = engine
        .recall(&query_emb, Some("session-reset".to_string()), true)
        .await
        .unwrap();
...
async fn test_recall_max_memories_cap(pool: PgPool) {
    ...
    let engine = RecallEngine::new(Arc::clone(&store), config, false);
    let result = engine
        .recall(&query_emb, Some("session-cap".to_string()), false)
        .await
        .unwrap();
...
async fn test_recall_extraction_tier(pool: PgPool) {
    ...
    let result = engine
        .recall(&query_emb, Some("session-extraction".to_string()), false)
        .await
        .unwrap();
...
async fn test_recall_no_extraction_tier(pool: PgPool) {
    ...
    let engine = RecallEngine::new(Arc::clone(&store), config, false);
    let result = engine
        .recall(&query_emb, Some("session-noextract".to_string()), false)
        .await
        .unwrap();
    ...
    assert!(ids.contains(&summary.id.as_str()), "summary should be recalled");
    assert!(
