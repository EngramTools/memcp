...
fn test_low_entropy_repetitive() {
    ...
    assert!(
        ent < 1.0,
        "repetitive string should have entropy < 1.0, got {ent}"
    );
}
...
fn test_high_entropy_real_key() {
    ...
    assert!(
        ent > 3.5,
        "real API key should have entropy > 3.5, got {ent}"
    );
}
...
fn test_low_entropy_placeholder() {
    ...
    assert!(
        ent < 3.5,
        "placeholder should have entropy < 3.5, got {ent}"
    );
}
...
fn test_two_distinct_chars() {
    ...
    assert!(
        (ent - 1.0).abs() < 0.01,
        "two distinct chars should have entropy ~1.0, got {ent}"
    );
}
