...
async fn test_feedback_useful_increases_stability(pool: PgPool) {
    ...
    let m = store
        .store(MemoryBuilder::new().content("Feedback useful test memory").build())
        .await
        .unwrap();

    // Get initial salience (defaults: stability=1.0)
    let before = store
        .get_salience_data(&[m.id.clone()])
        .await
        .unwrap();
...
async fn test_feedback_irrelevant_decreases_stability(pool: PgPool) {
    ...
    let m = store
        .store(MemoryBuilder::new().content("Feedback irrelevant test memory").build())
        .await
        .unwrap();
...
async fn test_feedback_invalid_signal_errors(pool: PgPool) {
    ...
    let m = store
        .store(MemoryBuilder::new().content("Feedback invalid signal test").build())
        .await
        .unwrap();
    ...
    assert!(
        result.is_err(),
        "invalid signal should return an error"
    );
    ...
    match result.unwrap_err() {
        memcp::errors::MemcpError::Validation { message, .. } => {
            assert!(
                ...
                "error message should mention the invalid signal: {}", message
            );
        }
...
async fn test_feedback_preserves_reinforcement_count(pool: PgPool) {
    ...
    let m = store
        .store(MemoryBuilder::new().content("Feedback count preservation test").build())
        .await
        .unwrap();
    ...
    let before = store
        ...
        .await
        .unwrap();
    let before_count = before.get(&m.id).cloned().unwrap_or_default().reinforcement_count;
    assert_eq!(before_count, 1, "reinforcement_count should be 1 after reinforce");
    ...
    let after = store
        ...
        .await
        .unwrap();
    let after_count = after.get(&m.id).cloned().unwrap_or_default().reinforcement_count;
    assert_eq!(
