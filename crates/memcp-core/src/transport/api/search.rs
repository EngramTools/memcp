...
pub async fn search_handler(
    ...
    if let Some(ms) = req.min_salience {
        if !(0.0..=1.0).contains(&ms) {
            return (
                StatusCode::BAD_REQUEST,
                Json(error_json("min_salience must be between 0.0 and 1.0")),
            );
        }
    }

    let effective_min = req
        .min_salience
    ...
    let cursor_position: Option<(f64, String)> = if let Some(ref c) = req.cursor {
        match decode_search_keyset_cursor(c) {
            ...
            Err(e) => {
                return (
                    StatusCode::BAD_REQUEST,
                    Json(error_json(&format!("Invalid cursor: {}", e))),
                );
            }
        }
    } else {
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            return (
                ...
                Json(error_json(&format!("Search failed: {}", e))),
            );
        }
    };
